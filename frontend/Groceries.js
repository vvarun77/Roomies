...
export function GroceryScreen({route}, components) {
	...
	useEffect(() => {
		if(!loading && error){
			async function addEmpty() {
				await addTodoHook({ variables: { input: {id: user.unsafeMetadata.groupid, groceries: []} } })
			}
			addEmpty().then()
			console.log(error)
		}

		if (!loading && data.getTodo.groceries.length != 0){
			setTasks(data.getTodo.groceries)
			console.log(data.getTodo.groceries)
		}
		...
		useEffect(() => {
			async function updateTodo() {
			  await updateTodoHook({
				variables: { input: { id: user.unsafeMetadata.groupid, groceries: tasks } },
			  });
