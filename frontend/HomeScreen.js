			...
			if(!loading && error) {
				async function addEmpty() {
					if(user.unsafeMetadata.groupid != null) {
                        ...
                        await addStatusHook({ variables: { input: {id: user.unsafeMetadata.groupid, todos: []} } })
					}
				}
				addEmpty().then()
				console.log(error)
    ...
    const renderItemGroup = ({ item, index }) => 
		(	
            <View style={{justifyContent:"center", alignItems:"center", width: 150}}>
                <View style={{borderRadius:50, backgroundColor:"#ccc0ef", height: 50, width: 50, justifyContent:"center", alignItems:"center"}}> 
                    <Image style={{height: 30, width: 30, tintColor:'white'}} source={Profile} />
                </View>
                
				<Text style={{fontFamily: Poppins, fontSize: 16, fontWeight: "bold", paddingTop: "2%",}}>{item.id}</Text>
                <Text style={{fontFamily: Poppins, fontSize: 16, fontWeight: "bold",}}>{item.status}</Text>
            </View>
		); 
	...
	return ( 
        <SafeAreaView style={styles.homeContainer}>
            ...
            <View style={styles.card}>
                ...
                <View style={{borderBottomWidth:3, borderBottomColor: "#ccc0ef", left:10, width:"95%", height:25}}/>
